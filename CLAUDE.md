...
- A lead is `{id, name, phone, email, service, source, stage, value, suburb, address, notes, createdAt, followUp, jobDate, jobTime, assignedTo, history[], archived?, d1?, d2?, d3?, trackedAt?}`.
...
- `d1`/`d2`/`d3` are plain booleans toggled by the "D1"/"D2"/"D3" checkboxes in the tracking row at the bottom of each Pipeline card (see `toggleFlag()`) — manual tick-off checkboxes for Aaron's own workflow (day-1/2/3 follow-up done), independent of stage. No stat/report currently reads them. (A `messaged` field/checkbox existed briefly but was removed — some older lead rows in Supabase may still carry a stray `messaged` key; harmless, just unused.)
- `trackedAt` is a timestamp (ms) set by `toggleFlag()` any time a D1/D2/D3 box is ticked — shown as "Updated {date, time}" under the tracking row (see `fmtDT()`) so Aaron can see at a glance when a lead's follow-up checkboxes were last touched.
